...
| 3b  | **Video playback**                   | 🟡 **FIX APPLIED (live verify pending)** | Signed R2 URL was blocked by CSP `media-src 'self' blob:`; R2 allowlisted in `media-src` (`fix/playback-media-src-csp`, PR #97). Proves the reported CSP error can't recur; a video has **not yet been seen to play** live — verification pending |
...
### 🟡 FIX APPLIED — live verify pending (2026-06-16) — Video playback blocked by CSP

**Fix applied (PR #97, `fix/playback-media-src-csp`):** option (a) below — allowlisted `https://*.r2.cloudflarestorage.com` in `media-src`. Option (b) (proxy/stream-through-app) was explicitly rejected: it adds egress + HTTP-Range complexity and routes video through Vercel; the short-lived signed-URL design is the de-facto architecture already in code and tests. A regression test (`next.config.test.ts`) locks the R2 host into both `media-src` and `connect-src`. Security review: PASS/LOW (scope identical to existing `connect-src` trust).

**⚠️ Not yet live-verified.** This proves the _reported_ `media-src` CSP error can no longer fire — it does **not** prove a video plays. The original symptom was "playback dead," and a second blocker (CORS, a separate console error, a player bug) could sit behind the CSP error, since the walkthrough stopped at the first error and never reached it. Verification = drive a browser against the preview/staging, confirm `currentTime` advances past 0 and `browser_console_messages` is clean, on **both** the authed `/videos/[id]` and guest `/guest/[token]` players. Until that's done, treat this as "fix applied," not "resolved."
