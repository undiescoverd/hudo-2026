...
- **[CSP] Playback blocked because `media-src` omits R2 (2026-06-16):** `next.config.js` allows R2 in `connect-src` (upload PUT works) but `media-src 'self' blob:` blocks the `<video>` loading the signed R2 URL → playback dead on staging. Decide: allowlist R2 in `media-src`, or stream bytes through the app (the latter matches the documented "playback via signing proxy / direct R2 URL never returned" rule — which the current direct-URL `/playback-url` already violates).
