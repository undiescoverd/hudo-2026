...
- **[CSP] Playback blocked because `media-src` omits R2 (2026-06-16, RESOLVED 2026-06-16):** `next.config.js` allowed R2 in `connect-src` (upload PUT works) but `media-src 'self' blob:` blocked the `<video>` loading the signed R2 URL → playback dead on staging. **Fix:** allowlisted `https://*.r2.cloudflarestorage.com` in `media-src` (one global CSP at `/(.*)` → fixes both authed `/videos/[id]` and guest `/guest/[token]` at once). Rejected the proxy/stream-through-app alternative (egress + HTTP-Range cost; signed-URL is the de-facto architecture in code+tests). Rule: any browser-facing R2 resource needs its host in **both** `connect-src` (fetch/XHR) **and** `media-src` (`<video>`/`<audio>`) — independent directives. Guard: `next.config.test.ts` asserts the R2 host stays in both (CSP contents are invisible to the mocked suite, so a string-level test is the only guard against silent regression).
