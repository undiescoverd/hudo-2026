...
- **[Seed] Staging "Staging Test Reel" video has no R2 object (2026-06-16):** seed video `7cb31754-187e-4f68-9a76-6a3a1973e080` (`r2_key seed/staging/7cb31754…/v1.mp4`) has a DB row but the file was never uploaded → GET returns `403 NoSuchKey`, so its `<video>` shows a format error (XML error body, not mp4). Masquerades as a playback bug but is a seed-data gap — the staging seed script creates the row without uploading bytes. Fix: re-run the seed upload or point the row at a real object. Use the actually-uploaded `55c07ab0…` ("hudo-test-upload") for playback smoke-tests until then. (Surfaced while live-verifying the CSP fix above.)
